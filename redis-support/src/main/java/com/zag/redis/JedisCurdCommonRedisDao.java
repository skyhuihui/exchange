...
import com.zag.redis.spi.JedisRedis;
import com.zag.redis.util.FieldMappingAssist;
...
public class JedisCurdCommonRedisDao<T extends BaseRedisObject<ID>, ID extends Serializable>  extends JedisRedis{
    ...
    Map<String, FieldMapping> fieldName_FieldMapping_Annotation_Map = null;
    Map<FieldMapping, FieldMappingAssist<T, ID>> fieldInFieldMappingMap = null;
            ...
            if (fields != null && fields.length > 0) {
                for (Field field : fields) {
                    if (!Modifier.isFinal(field.getModifiers())) {
                        ...
                        //获取字段中包含的注解
                        FieldMapping fieldFieldMapping = field.getAnnotation(FieldMapping.class);
                        boolean isFieldMapping = (fieldFieldMapping == null ? false : true);
                        if (isFieldMapping) {
                            if (fieldName_FieldMapping_Annotation_Map == null) {
                                fieldName_FieldMapping_Annotation_Map = new HashMap<>();
                            }
                            if (fieldInFieldMappingMap == null) {
                                fieldInFieldMappingMap = new HashMap<>();
                            }

                            fieldInFieldMappingMap.put(fieldFieldMapping, new FieldMappingAssist<>(
                                    field.getName(),
                                    StringUtils.isBlank(fieldFieldMapping.prefix()) ? getKeyPrefix() + SEPARATOR + field.getName() : fieldFieldMapping.prefix() + SEPARATOR + field.getName(),
                                    parser.parseExpression(fieldFieldMapping.key()
                                    )));
                            fieldName_FieldMapping_Annotation_Map.put(field.getName(), fieldFieldMapping);
                        }
    ...
    public void save(T ro) {
        ...
        if (MapUtils.isNotEmpty(fieldInFieldMappingMap)) {
            for (FieldMapping fieldMapping : fieldInFieldMappingMap.keySet()) {
                FieldMappingAssist<T, ID> field = fieldInFieldMappingMap.get(fieldMapping);
                set(field.getKey(ro), RedisUtil.toByteArray(ro.getId()));
                if (logger.isDebugEnabled()) {
                    logger.debug("mapping set ({},{})", field.getKey(ro), ro.getId());
                }
            }
        }
